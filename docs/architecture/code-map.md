...
| [`review.rs`](../../src/review.rs) | `rationale review` (CLI): confirmación humana de propuestas, una a la vez, con efecto práctico visible. Única función del sistema que añade una `Approval` real y promueve `proposals/` → `records/`. | No corre dentro del servidor MCP — necesita un humano interactivo. |
