export const landingCopy = {
  en: {
    ...
    heroMeta: "v1.1.0 stable · Git and Codebase Memory · no account · no telemetry · 127.0.0.1",
    ...
    skillInstallLabel: "For any other agent that reads skills",
    skillInstallNote: "`rationale init` and `install-agent` already install it for Claude Code and Codex.",
    ...
    codexNote: "Codex reads the protocol from `AGENTS.md`, calls the MCP tools and loads the skill from `.agents/skills/`. `$rationale` calls it by name.",
    ...
    releaseTitle: "1.1 is stable. Here is what that means.",
    releaseBody: "The full loop has been implemented, tested and used on Rationale’s own repository since 1.0, and 1.1 adds the rationale skill.",
    releaseVerified: [
      "386 Rust tests on macOS, Linux and Windows, with Clippy and RustSec",
      "The skill checked against its directory, the Agent Skills limits and the capture gate",
      "Every tag re-verified from source before it is packaged",
      "1.0: packaged binary, registration migration and an end-to-end dogfood change",
    ],
    releaseOpen: [
      "Lexical intent-conflict polarity is a noisy hint",
      "Skill selection is probabilistic; the protocol stays the floor",
    ...
    footerStatus: "v1.1.0 · no remote telemetry",
  },
  es: {
    ...
    heroMeta: "v1.1.0 estable · Git y Codebase Memory · sin cuenta · sin telemetría · 127.0.0.1",
    ...
    skillInstallLabel: "Para cualquier otro agente que lea skills",
    skillInstallNote: "`rationale init` e `install-agent` ya lo instalan para Claude Code y Codex.",
    ...
    codexNote: "Codex lee el protocolo en `AGENTS.md`, llama las herramientas MCP y carga el skill desde `.agents/skills/`. `$rationale` lo llama por nombre.",
    ...
    releaseTitle: "1.1 es estable. Esto es lo que significa.",
    releaseBody: "El ciclo completo está implementado, probado y en uso sobre el propio repositorio de Rationale desde 1.0, y 1.1 agrega el skill rationale.",
    releaseVerified: [
      "386 tests de Rust en macOS, Linux y Windows, con Clippy y RustSec",
      "El skill verificado contra su directorio, los límites de Agent Skills y el gate de captura",
      "Cada tag se vuelve a verificar desde el código antes de empaquetarse",
      "1.0: binario empaquetado, migración del registro y un cambio real de dogfood de extremo a extremo",
    ],
    releaseOpen: [
      "La polaridad léxica de los conflictos es una pista ruidosa",
      "La selección del skill es probabilística; el protocolo sigue siendo el piso",
    ...
    footerStatus: "v1.1.0 · sin telemetría remota",
  },
} as const;
